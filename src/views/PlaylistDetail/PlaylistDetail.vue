<template>
  <div class="playlistDetail">
    ...
    <div ref="box" class="playlistScroll">
...
import { returnHistoryMixin } from "@/common/mixin.js";
...
export default {
  ...
  methods: {
    ...
    autoPlay(id) {
      this.musicUrl.forEach((item, index) => {
        if (item.id === id) {
          this.$store.commit("changeMusic", this.musicUrl[index].url);
          console.log(this.musicUrl[index].url);
        }
      });

      // console.log(this.$store.state.currentMusic);

      // console.log(this.musicUrl[index]);
    }
  },
  mixins: [returnHistoryMixin],
  created() {
